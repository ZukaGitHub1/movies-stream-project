...
import '../../styles/SingleIdPageDesign.scss'
...
const SingleidPageDesign = ({list }) => {
  ...
  return (
    <div key={id} className='single-movie-div   bg-slate-900 '>
    ...
    <h2 className="flex justify-center gap-2">Genre :{genres && genres.map(index => (<div key={index.id}>{index.name}</div>))}</h2>
